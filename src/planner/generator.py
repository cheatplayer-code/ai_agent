...
def generate_plan(
    ...
    """Generate a fixed deterministic plan for the pipeline."""
    steps = [
        make_plan_step(
        ...
        make_plan_step(
            step_id="step_4_run_analysis_tools",
            step_type=STEP_TYPE_RUN_ANALYSIS_TOOLS,
            depends_on=["step_3_run_dq_suite"],
        ),
        make_plan_step(
            step_id="step_5_verify_claims",
            step_type=STEP_TYPE_VERIFY_CLAIMS,
            params={"claims_provided": claims_provided},
            depends_on=["step_4_run_analysis_tools"],
        ),
        make_plan_step(
            step_id="step_6_build_report",
            step_type=STEP_TYPE_BUILD_REPORT,
            depends_on=["step_5_verify_claims"],
        ),
    ]
    return make_plan(steps=steps, plan_id="deterministic_plan_v1")
